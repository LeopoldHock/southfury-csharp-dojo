...
// Complete the below code to have one Person sit on a Chair. Mak
...
namespace southfury_csharp_dojo
{
    class Program
    {
        static void Main()
        {
            ...
            // Make either Tanner or Andra get up for Leo so Leo can sit.
            // However, there is aproblem with the LeaveCurrentChair() method of the Person class. Oh no, tt throws an exception!
            ...
            foreach (Person person in people)
            {
                ...
                {
                    Console.WriteLine(person.name + " sits on the " + person.currentChair.color);
                    if (person.currentChair.occupant == person)
                    {
    ...
    class Person
    {
        ...
        public void GetANewChair(string color)
        {
            // This method is not finished. Complete it and use it in your program! 
            Chair newChair = new Chair(color);
            Console.WriteLine(this.name + " speaks a few magic words and makes a " + newChair.color + " appear. Incredible!");
        }
